...
def get_town_data(file_name):
    ...
    try:
        town_data = pd.read_csv(file_name, header=0,
                                usecols=['zip', 'type', 'town', 'state',
                                         'latitude', 'longitude'],
                                dtype={'zip': str, 'type': str, 'town': str,
                                       'state': str, 'latitude': str,
                                       'longitude': str})
        print("File Type: CSV")
...
def get_towns_within_range(destination, max_range):
    ...
    print('Downloading Data for towns within {} miles of {}'.format(max_range,
                                                                    destination))
    ...
    for x in range(0, len(lat_longs), batch_limit):
        batch.append(
            gmaps.distance_matrix(lat_longs[x:x + batch_limit], destination,
                                  ode=mode,
    ...
    for each in batch:
        filtered_batch.append(
            {key: each[key] for key in selected_keys if key in each})
    ...
    for row in filtered_batch:
        ...
        for each in row.itertuples():
            ...
            if each.rows['elements'][0]['status'] != "ZERO_RESULTS":
                ...
                town = ' '.join(town)
                distance = float(
                    each.rows['elements'][0]['distance']['text'].split(" ")[0])
    ...
    file_name = os.path.join(DATA_DIR,
                             "town_data_" + str(max_range) + "mi.xlsx")
