...
def calculateCombinedScores(fileName):
    ...
    for row in range(len(commuteScores)):
        ...
        data[town] = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        distance = commuteScores.iloc[row, 0]
        duration = commuteScores.iloc[row, 1]
        distanceScore = commuteScores.iloc[row, 2]
        durationScore = commuteScores.iloc[row, 3]
        commuteScore = commuteScores.iloc[row, 4]
        
        data[town][1] = distance
        data[town][2] = duration
        data[town][5] = commuteScore
        ...
        if town in schoolScores.index.values:
            data[town][0] = town
            data[town][7] = schoolScores['Weighted Score'][town]
        # if not in school scores, look in school data
            ...
            if len(district) > 0 and district[0] in schoolScores.index.values:
                ...
                data[town][7] = schoolScores['Weighted Score'][district[0]]
            else:
        ...
        if town in data:   
            ...
            currCombinedScore += calculateWeightedScore(['Commute', data[town][5]])
            currCombinedScore += calculateWeightedScore(['House', data[town][6]])
            currCombinedScore += calculateWeightedScore(['School', data[town][7]])
